...
export interface WpBalance {
  ...
  bank: number;
  /**
   * Общий баланс макулатуры = наличка + безнал. Это основной остаток
   * (по факту один денежный счёт). Сторонние пополнения в него НЕ входят.
   */
  common: number;
  /** Сторонние пополнения — отдельный счёт, вне общего баланса. */
  third_party: number;
  /**
   * Всего по всем счетам, включая сторонние (common + third_party).
   * НЕ является «общим балансом» — используется только там, где нужно
   * показать сумму с учётом сторонних (явно подписанная плашка).
   */
  total: number;
}
