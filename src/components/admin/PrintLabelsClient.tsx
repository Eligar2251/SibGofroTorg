...
// ширина 60 мм × высота 40 мм. Содержимое: QR слева (≈36 мм),
// справа — название и цена. Штрихкод по-прежнему не печатаем
// (на термоэтикетках мелкие штрихи плывут) — ТОЛЬКО QR + подпись.
// qrSize: 36 мм при 203 dpi термоголовки ≈ 287 px — берём 280.
const TAPE_DIM = {
  widthMm: 60,
  heightMm: 40,
...
export function PrintLabelsClient({
  ...
  return (
    <div className="qrprint">
      ...
      <div className="qrprint__filters no-print">
        ...
        {mode === "tape" && (
          <div className="qrprint__hint">
            <strong>Этикетка 40×60 мм (альбомная) — один QR-код на
            этикетку.</strong> Каждая этикетка отправляется на принтер
            отдельной страницей ровно 60×40 мм. В диалоге печати
            выберите ваш термопринтер (Xprinter XP-365B / Brother /
            MUNBYN / Mercury), размер бумаги <strong>60×40 мм</strong> (или
            «40×60 альбомная»), поля — «Нет», масштаб — «100%» /
            «По размеру страницы». На самой этикетке: QR слева,
            справа название и цена.
          </div>
        )}
      </div>
       ...
       * выше + page-break-after в CSS), поэтому на одну физическую
       * этикетку попадает ровно ОДИН QR-код. Содержимое: QR слева,
       * справа название и цена (штрихкод на термо плывёт — не печатаем).
       */}
      {mode === "tape" && (
        <div
