...
function cleanPromotion(body: Record<string, unknown>) {
  const linkType = ["product", "url", "none"].includes(String(body.linkType)) ? String(body.linkType) : "none";
  return {
    title: String(body.title || "").trim().slice(0, 200),
    subtitle: body.subtitle ? String(body.subtitle).slice(0, 500) : null,
    badge: body.badge ? String(body.badge).slice(0, 80) : null,
    image_url: body.imageUrl ? String(body.imageUrl).slice(0, 1000) : null,
    link_type: linkType,
    product_id: linkType === "product" && body.productId ? String(body.productId) : null,
    link_url: linkType === "url" && body.linkUrl ? String(body.linkUrl).slice(0, 1000) : null,
...
export async function PUT(
  ...
  try {
    ...
    const db = getAdminDb();
    await db.from("promotions").update(update).eq("id", id);
